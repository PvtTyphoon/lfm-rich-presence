...
if(store.get("username")) document.getElementById("username").value = store.get("username");
if(store.get("apiKey")) document.getElementById("username").value = store.get("username");
...
document.getElementById("run").addEventListener("click", () => {
  rp.on("ready", async () => {
      ...
      if(!username) return consoleDiv.innerHTML += `<p class="err">[x] Please provide a last.fm username.</p>`
      if(!apiKey) return consoleDiv.innerHTML += `<p class="err">[x] Please provide a last.fm API key or reload this page (ctrl + r) to use the default key.</p>`
